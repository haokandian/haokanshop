...
class PosterGoodsService
{
    ...
    public static function GoodsCreateMiniWechat($params = [])
    {
        ...
        $goods_params = [
            ...
            'field' => 'id,title,simple_desc,images,price,original_price,add_time',
            'm'     => 0,
            'n'     => 1,
        ];
        ...
        // 获取商品二维码
        $qrcode = self::UserCreateMiniWechatQrcode($goods, $user);
        if($qrcode['code'] != 0)
        {
            return $qrcode;
        }
        ...
        // 海报地址
        $poster_path = 'static'.DS.'upload'.DS.'images'.DS.'plugins_distribution'.DS.'poster_goods'.DS.date('Y', $goods['add_time']).DS.date('m', $goods['add_time']).DS.date('d', $goods['add_time']).DS;
        $poster_filename = date('YmdHis', $goods['add_time']).$goods['id'].$user['id'].'.png';
        ...
        // 静态资源路径
        $static_path = ROOT.'public'.DS.'static'.DS.'plugins'.DS.'images'.DS.'distribution'.DS;
        ...
        $config = [
            ...
            'text' => [
                ...
                [
                    'text' => empty($poster_config['bottom_right_text']) ? '微信扫码购买' : $poster_config['bottom_right_text'],
                    'left' => 300,
                    'top' => 770,
                    'font_size' => 14,
        ...
        // 生成商品海报
        if(self::CreatePoster($config, $poster_dir))
        {
            return DataReturn('商品海报创建成功', 0, ResourcesService::AttachmentPathViewHandle(DS.$poster_path.$poster_filename));
        }
    ...
    private static function CreatePoster($config = [], $filename = '')
    {
        ...
        if(!empty($config['text']))
        {
            foreach($config['text'] as $key=>$val)
            {
                ...
                {
                    // 文字水平居中实质
                    $font_box = imagettfbbox($val['font_size'], 0, $val['font_path'], $text);
                ...
                imagettftext($image_res, $val['font_size'], $val['angle'], $val['left'], $val['top'], $font_color, $val['font_path'], $text);
            }
        }
        //生成图片
        if(!empty($filename))
        {
            //保存到本地
            $res = imagejpeg($image_res, $filename, 90);
     ...
     */
    private static function UserCreateMiniWechatQrcode($goods, $user)
    {
        // 商品二维码地址
        $path = 'static'.DS.'upload'.DS.'images'.DS.'plugins_distribution'.DS.'poster_goods_qrcode'.DS.date('Y', $goods['add_time']).DS.date('m', $goods['add_time']).DS.date('d', $goods['add_time']).DS;
        ...
        {
            return DataReturn('获取成功', 0, DS.$path.$filename);
        }

        // 目录不存在则创建
        if(\base\FileUtil::CreateDir(ROOT.'public'.DS.$path) !== true)
        {
            return DataReturn('商品二维码目录创建失败', -1);
        }
        ...
        $wx_params = [
            ...
            'scene' => 'goods_id='.$goods['id'].'&referrer='.$referrer,
            'width' => 300,
        ];
        ...
        // 保存二维码
        if(@file_put_contents($qrcode_dir, $ret['data']) !== false)
        {
            return DataReturn('获取成功', 0, DS.$path.$filename);
        }
